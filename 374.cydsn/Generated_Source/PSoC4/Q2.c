...
#include "Q2.h"
...
#if defined(Q2__PC)
    #define Q2_SetP4PinDriveMode(shift, mode)  \
    do { \
        Q2_PC =   (Q2_PC & \
                                (uint32)(~(uint32)(Q2_DRIVE_MODE_IND_MASK << \
                                (Q2_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Q2_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Q2_SetP4PinDriveMode(shift, mode)  \
        do { \
            Q2_USBIO_CTRL_REG = (Q2_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Q2_DRIVE_MODE_IND_MASK << \
                                    (Q2_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Q2_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Q2_SetP4PinDriveMode(Q2__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(Q2_DR & (uint8)(~Q2_MASK));
    drVal = (drVal | ((uint8)(value << Q2_SHIFT) & Q2_MASK));
    Q2_DR = (uint32)drVal;
}
...
uint8 Q2_Read(void)
{
    return (uint8)((Q2_PS & Q2_MASK) >> Q2_SHIFT);
}
...
{
    return (uint8)((Q2_DR & Q2_MASK) >> Q2_SHIFT);
}
...
void Q2_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Q2_INTCFG & (uint32)(~(uint32)position);
    Q2_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(Q2_INTSTAT & Q2_MASK);
	Q2_INTSTAT = maskedStatus;
    return maskedStatus >> Q2_SHIFT;
}
